...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    %% ── Steps ────────────────────────────────────────────────
    \node[stepbox, fill=blue!10]   (t1) at (-4.5,-0.8) {Type \texttt{@ai ...}};
    \node[stepbox, fill=blue!10]   (t2) at (-4.5,-1.8) {emit("project-message")};
    \node[stepbox, fill=green!10]  (t3) at (0,   -2.5) {Detect \texttt{@ai} prefix};
    \node[stepbox, fill=green!10]  (t4) at (0,   -3.4) {Extract prompt};
    \node[stepbox, fill=green!10]  (t5) at (0,   -4.2) {Broadcast raw msg};
    \node[stepbox, fill=orange!10] (t6) at (4.5, -4.9) {Process \& generate};
    \node[stepbox, fill=orange!10] (t7) at (4.5, -5.8) {Return JSON response};
    \node[stepbox, fill=green!10]  (t8) at (0,   -6.2) {\texttt{io.to(room).emit}};
    \node[stepbox, fill=blue!10]   (t9) at (-4.5,-7.0) {Render AI message};
